		  ...
		  margin:(CGSize)marg
{
	...
	self.frame = CGRectMake(tl.x, tl.y, sysBounds.width + 2 * margin.width, sysBounds.height + 2 * margin.height);
	[self hideCursor];
	[self setNeedsDisplay];
...
-(void)drawInContext:(CGContextRef)ctx
{
	...
	else if (_drawFlags != 0) // use drawFlags if no coloured items
	{
		...
		[system drawWithContext:ctx at:tl magnification:zoom colourRender:render];
	}
	else
...
-(void)updateLayout:(CGContextRef)ctx prevState:(sscore_state_container *)prevstate newState:(const sscore_state_container *)newstate
{
	[system updateLayout:ctx newState:newstate];
	CGSize sysBounds = system.bounds;
	CGRect frame = self.frame;
	frame.size = CGSizeMake(sysBounds.width + 2 * margin.width, sysBounds.height + 2 * margin.height);
	self.frame = frame;
	[self setNeedsDisplay];
}
...
-(void)change:(sscore_state_container *)prevstate newstate:(sscore_state_container *)newstate reason:(int)reason
{
	...
	bool changed = changedBars(self.system.barRange, prevstate, newstate)
				|| sscore_edit_headerChanged(prevstate, newstate);
	if (changed)
	{
		UIGraphicsBeginImageContextWithOptions(CGSizeMake(10,10), YES/*opaque*/, 0.0/* scale*/);
		CGContextRef ctx = UIGraphicsGetCurrentContext();
		[self updateLayout:ctx prevState:prevstate newState:newstate];
		UIGraphicsEndImageContext();
