...
extern "C" {
	...
	/*!
	 @abstract an invalid bar index
	 */
	#define sscore_pd_kInvalidBarIndex (-1)

	/*!
	 @abstract special value to indicate beyond the last bar in the piece
	 */
	#define sscore_pd_kAfterEndBarIndex (-2)


	#define sscore_pd_MaxNoteSamples 128
	
	#define sscore_pd_kMaxBeatsInBar 32
	...
	enum sscore_pd_articulation_flags_e {
	...
	/*!
	 ...
	 @abstract information about a note for playing
	 @discussion NB The right-hand note of a tie is included in the duration of the left hand note and omitted from the list
	 @discussion If the note in the XML is unpitched and has a defined instrument id then a lookup
	 is performed in the table of |midi-instrument| in |score-part| in the xml file to find a
	 matching instrument, and the |midi-unpitched| value is returned in midipitch, with the |midi_channel| value in midichannel.
	 If this is channel 10, according to the MIDI spec 1.1 we can play a set of standard MIDI percussion sound samples directly
	 */
	typedef struct sscore_pd_note
	{
		int midipitch;	// 60 = C4. 0 = unpitched (eg rest) (also used as beat index for metronome)
						// if |midi-instrument| defined in XML this is the midi-unpitched note.
		...
		int staffindex; // index of the staff containing this note. 0 is top (or only) staff
		unsigned articulation_flags; // bit flags sscore_pd_articulation_flags_e. A rest has pitch = 0 and .._rest_flag set
		int midichannel; // non-zero if |midi-instrument| defined in XML. = 10 for midi percussion (by MIDI spec)
		int midiprogram; // non-zero if |midi-instrument| defined in XML.
		unsigned dummy[2]; // future
	} sscore_pd_note;
	...
	{
		int midipitch;	// 60 = C4. 0 = unpitched
		int duration;	// (ms) may be longer than a bar if tied
		unsigned articulation_flags; // bit flags sscore_pd_articulation_flags_e
		unsigned dummy[1];
	...
	typedef struct sscore_pd_usertempo
	{
		...
		/*!
		 get_slash_graces_per_crotchet
		 if func pointer != NULL and result > 0 get duration of a grace-slash as 1/duration of a crotchet (quarter note)
		 eg return 4 for grace-slash duration = semiquaver (16th note)
		 */
		sscore_pd_int_fn get_slash_graces_per_crotchet;
		
		unsigned dummy[7];
	} sscore_pd_usertempo;
	
	typedef bool (*sscore_pd_bool_fn)(void *context);
	typedef bool (*sscore_pd_bool_intfn)(void *context, int);
	typedef bool (*sscore_pd_bool_intintfn)(void *context, int, int);
	typedef int (*sscore_pd_int_intfn)(void *context, int);
	typedef float (*sscore_pd_float_intfn)(void *context, int);

	/*!
	 @struct sscore_pd_controls
	 @abstract link to UI which defines playing parameters for midi output
	 */
	typedef struct sscore_pd_controls
	{
		void *context;
		sscore_pd_bool_intfn partEnabled; // return true to output notes in this part into MIDI file
		sscore_pd_int_intfn midiProgramForPart; // define MIDI program change value for part if > 0
		sscore_pd_bool_intintfn partStaffEnabled; // return true to output notes in part and staff
		sscore_pd_bool_fn metronomeEnabled; // return true to output notes for metronome ticks
		sscore_pd_int_fn midiKeyForMetronome; // return MIDI key number for metronome tick in percussion channel 10 (see MIDI Spec 1.1, Appendix 1.5 - General MIDI Percussion Key Map)
		sscore_pd_float_intfn partVolume; // return 0.0 .. 1.0 part volume
		sscore_pd_float_fn metronomeVolume;  // return 0.0 .. 1.0 metronome volume
		unsigned dummy[60]; // for future enhancement
	} sscore_pd_controls;

	/*!
	 @struct sscore_pd_timinginfo
	 @abstract specification of beat timing for media synchronisation 
	 */
	typedef struct sscore_pd_timinginfo
	{
		int mediaStart_ms;			// if non-zero specifies a delay before starting synchronised media
		int numbars;
		
		struct {
			int barIndex;			// the 0-based index of this bar
			int duration_ms;		// duration of bar in ms
			int numSubsequentBeats; // = 3 for a 4-beat bar
			
			struct {
				int time_ms;		// time of subsequent beat from start (first beat) of bar
				
				unsigned dummy[4];
			} beat[sscore_pd_kMaxBeatsInBar];
			
			unsigned dummy[4];
		} bar[sscore_kMaxBars];
		
		unsigned dummy[64];
	} sscore_pd_timinginfo;

	/*!
	 @struct sscore_pd_midi_params
	 @abstract define various midi parameters
	 */
	typedef struct sscore_pd_midi_params
	{
		int loopStartBarIndex; // define loop start bar index
		int loopEndBarIndex; // define end loop bar index
		int loopNumRepeats; // 0 for no play loop
		unsigned dummy[128];// future expansion
	} sscore_pd_midi_params;

	/*!
	 @typedef sscore_playdata
	...
	/*!
	 @function sscore_pd_newplaydata_singlepart
	 @abstract create playdata for single part/single bar only
	 @param sc the score
	 @param partIndex the part index
	 @param barIndex the bar index
	 @param usertempo callbacks for realtime user defined tempo
	 @param key the key defining licenses owned by the user.
	 @return pointer to new playdata
	 */
	EXPORT const sscore_playdata *sscore_pd_newplaydata_singlepart(const sscore *sc,
																  int partIndex, int barIndex,
																  const sscore_pd_usertempo *usertempo,
																  const sscore_libkeytype *key);
	
	/*!
	 @function sscore_pd_newplaydata_singlebar
	 @abstract create playdata for single part/single bar only
	 @param sc the score
	 @param partIndex the part index
	 @param barIndex the bar index
	 ...
	 @return pointer to new playdata
	 */
	EXPORT const sscore_playdata *sscore_pd_newplaydata_singlebar(const sscore *sc,
																  int partIndex, int barIndex,
																  const sscore_pd_usertempo *usertempo,
																  const sscore_libkeytype *key);
	
	/*!
	 @function sscore_pd_newplaydata_singlechord
	 @abstract create playdata for single note or chord only
	 @param sc the score
	 @param partIndex the part index
	 @param barIndex the bar index
	 @param noteId the sscore_item_handle for the note
	 @param key the key defining licenses owned by the user.
	 @return pointer to new playdata
	 */
	EXPORT const sscore_playdata *sscore_pd_newplaydata_singlechord(const sscore *sc,
																  int partIndex, int barIndex, sscore_item_handle noteId,
																  const sscore_libkeytype *key);

	/*!
	 @function sscore_pd_newplaydata_withtiming
	 @abstract create playdata which is synchronised to the timing array
	 @param sc the score
	 @param usertempo callbacks for realtime user defined tempo
	 @param timing definition of timing to synchronise the playdata
	 @param key the key defining licenses owned by the user. If this key does not include a playdata licence then
	 this will operate in evaluation mode and a limited number of notes will be returned after
	 the app is started, after which every sscore_pd_note returned from sscore_pd_bni_getnote will have
	 all fields set to 0
	 @param error if non-null this will be set to an error value
	 @return pointer to new playdata
	 */
	EXPORT const sscore_playdata *sscore_pd_newplaydata_withtiming(const sscore *sc,
																   const sscore_pd_usertempo *usertempo,
																   const sscore_pd_timinginfo *timing,
																   const sscore_libkeytype *key,
																   enum sscore_error *error);
	...
	/*!
	 @function sscore_pd_numstavesforpart
	 @return number of staves in part
	 */
	EXPORT int sscore_pd_numstavesforpart(const sscore_playdata *pd, int partindex);

	/*!
	 @function sscore_pd_hasdataforpart
	 @return true if the part is enabled for play
	 */
	EXPORT bool sscore_pd_hasdataforpart(const sscore_playdata *pd, int partindex);

	/*!
	 @function sscore_pd_hasdataformetronome
	 @return true if the metronome is enabled for play
	 */
	EXPORT bool sscore_pd_hasdataformetronome(const sscore_playdata *pd);
	...
	/*!
	 @function sscore_pd_virtualinstrumentlibrary
	 @return the string value of the &lt;virtual-instrument&gt;&lt;virtual-library&gt; element for the part - empty if non-existent
	 */
	EXPORT const char *sscore_pd_virtualinstrumentlibrary(const sscore_playdata *pd, int partindex);
	
	/*!
	 @function sscore_pd_virtualinstrumentname
	 @return the string value of the &lt;virtual-instrument&gt;&lt;virtual-name&gt; element for the part - empty if non-existent
	 */
	EXPORT const char *sscore_pd_virtualinstrumentname(const sscore_playdata *pd, int partindex);
	
	/*!
	 @function sscore_pd_midiprogramchange
	 @discussion if the &lt;virtual-instrument&gt;&lt;virtual-library&gt; element contains "General MIDI" then lookup the
	 string value of the &lt;virtual-instrument&gt;&lt;virtual-name&gt; element in Program Change table of standard instrument
	 names in MIDI spec 1.1 http://www.midimusic.org.uk/tech/midispec.html
	 of the
	 @return the program change value if virtual instrument is correctly defined in the MusicXML, else return 0
	 */
	EXPORT int sscore_pd_midiprogramchange(const sscore_playdata *pd, int partindex);
	...
	/*!
	 @function sscore_pd_updatetempo
	 @abstract notification of tempo change
	 */
	EXPORT void sscore_pd_updatetempo(const sscore_playdata *pd);
	...
	/*!
	 @function sscore_pd_loopbegin
	 @abstract get an iterator which goes repeatedly from startBarIndex to loopBarIndex for the given number of repeats
	 @discussion licence playloop_capable is required to use this function
	 @param pd the playdata
	 @param startBarIndex the index for the bar to loop from and return to at the start of each loop
	 @param loopBarIndex the index for the last bar to play in the loop
	 @param numRepeats the number of loops
	 @return an iterator which loops between the specified bars
	 */
	EXPORT sscore_pd_bar_iterator sscore_pd_loopbegin(const sscore_playdata *pd, int startBarIndex, int loopBarIndex, int numRepeats);

	/*!
	 @function sscore_pd_loopend
	 @abstract get an iterator for the end of looping of loopbegin
	 @param pd the playdata
	 @param begin the start iterator returned from sscore_pd_loopbegin
	 @return an iterator to the end of the loop
	 */
	EXPORT sscore_pd_bar_iterator sscore_pd_loopend(const sscore_playdata *pd, const sscore_pd_bar_iterator *begin);
	...
	/*!
	 ...
	 If bi is a looping iterator and it is at the last bar in the loop it goes to the first bar in the loop
	...
	/*!
	 @function sscore_pd_bi_canchangebar
	 @abstract true if sscore_pd_bi_changebar will succeed with these parameters
	 @param bi the iterator
	 @param barindex the new bar index
	 */
	EXPORT bool sscore_pd_bi_canchangebar(const sscore_pd_bar_iterator *bi, int barindex);

	/*!
	 @function sscore_pd_bi_changebar
	 @abstract move the bar iterator to the given bar index. This will fail if it is outside the range of any defined loop on this iterator
	...
	/*!
	 ...
	 @param bi1 iterator 1
	 @param bi2 iterator 2
	...
	/*!
	 ...
	 @abstract compare 2 iterators
	 @param bni1 iterator 1
	 @param bni2 iterator 2
	...
	/*!
	 @function sscore_pd_create_midifile_control
	 @abstract create a MIDI file from the playdata
	 @discussion licence playloop_capable is required to use the playloop function
	 @param pd the playdata
	 @param midifilename full pathname for the midifile
	 @param controls define which parts and which staves are enabled for output to the file
	 @param params other optional parameters, may be NULL if unused
	 @return error
	 */
	EXPORT enum sscore_error sscore_pd_create_midifile_control(const sscore_playdata *pd,
															   const char *midifilename,
															   const sscore_pd_controls *controls,
															   const sscore_pd_midi_params *params);
