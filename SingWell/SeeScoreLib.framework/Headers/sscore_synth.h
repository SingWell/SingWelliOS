...
#include <stdbool.h>
	...
	enum sscore_sy_instrumentfamily
	{
		sscore_sy_instrumentfamily_undefined,
		sscore_sy_instrumentfamily_hammeredstring, // eg piano
		sscore_sy_instrumentfamily_bowedstring, // eg violin
		sscore_sy_instrumentfamily_pluckedstring, // eg guitar
		sscore_sy_instrumentfamily_woodwind,
		sscore_sy_instrumentfamily_brass,
		sscore_sy_instrumentfamily_percussion,
		sscore_sy_instrumentfamily_midi_percussion
	};
	
	enum sscore_sy_sampledinstrument_flags{
		sscore_sy_suppressrmscompensation_flag = 1 // set this eg for a percussion instrument where we do not want auto compensation for volume differences between samples
	};
	
	/*!
	 @enum sscore_sy_sampledinstrument_samples_flags
	 @abstract bit flags for specifying set of samples for instrument articulation
	 */
	enum sscore_sy_sampledinstrument_samples_flags
	{
		/*! (required) normal note sample files must be named (base_filename).(midipitch).extn */
		sscore_sy_normalsamples_flag = 1,

		/*! (optional) short note sample files must be named (base_filename).short.(midipitch).extn */
		sscore_sy_shortsamples_flag = 1<<1,
		
		/*! (optional) pizz sample files must be named (base_filename).pizz.(midipitch).extn */
		sscore_sy_pizzicatosamples_flag = 1<<2,
	};
	...
	/*!
	 @enum sscore_sy_synthesizedinstrument_type
	 @abstract define types of synthesized instrument
	 */
	enum sscore_sy_synthesizedinstrument_type
	{
		sscore_sy_tick1,
		sscore_sy_tick2,
		sscore_sy_tick3,
		sscore_sy_pitched_waveform_instrument
	};

	/*!
	 @enum sscore_sy_synthesizedinstrument_flags
	 @abstract define flags for synthesized instrument voice
	 */
	enum sscore_sy_synthesizedinstrument_flags
	{ sscore_sy_synthesizedinstrument_dummy };
	
	
	typedef bool (*sscore_sy_bool_intfn)(void *context, int);
	typedef bool (*sscore_sy_bool_intintfn)(void *context, int, int);
	...
	typedef int (*sscore_sy_int_fn)(void *context);

	enum sscore_sy_synthesizedinstrument_waveform // for waveform instrument
	{
		sscore_sy_sine, // sine wave
		sscore_sy_square, // square wave
		sscore_sy_triangle // triangle wave, symmetry determined by parameters, extreme is sawtooth
	};
	
	/*!
	 @abstract type of a function to return a waveform type for the waveform generator
	 */
	typedef enum sscore_sy_synthesizedinstrument_waveform (*sscore_sy_waveformfn)(void *context);
	
	/*!
	 @abstract type of a function to convert a midi pitch to a frequency (Hz)
	 */
	typedef float (*sscore_sy_pitchtofreqfn)(int midipitch, void *context);

	/*!
	 @struct sscore_sy_synthSoundParameterControls
	 @abstract link to UI to define parameters for synthesized sounds so they can be actively changed while playing
	 */
	typedef struct sscore_sy_synthSoundParameterControls
	{
		sscore_sy_pitchtofreqfn pitch_converter_func;	// function which returns a Hz frequency value for a midi pitch so you can define different temperaments. If NULL equal temperament with A4 = 440 is used as a default
		void *pitch_context;							// context for pitch_converter_func. If NULL equal temperament with A4 = 440 is used as a default
		sscore_sy_waveformfn waveform_func;				// function to return a waveform type for the waveform generator (uses parameter_context)
		sscore_sy_floatfn waveform_symmetry_func;		// function to return waveform symmetry for square/triangle waveforms [0.0..1.0] (uses parameter_context)
		sscore_sy_int_fn waveform_risefall_func;		// function to return a rise-fall time (samples) for the square wave (uses parameter_context)
		void *parameter_context;
		sscore_sy_int_fn dummy[6];
	} sscore_sy_synthSoundParameterControls;
	...
	typedef struct sscore_sy_synthesizedinstrumentinfo
	{
		...
		int unused;
		float volume;					// for adjustment of balance with other instruments
		enum sscore_sy_synthesizedinstrument_type type; // specific (eg tick2 or waveform)
		int attack_time_ms;				// 10% to 90% sigmoid attack time (may be ignored for some instruments)
		int decay_time_ms;				// 90% to 10% sigmoid decay time (ditto)
		unsigned flags;					// sscore_sy_synthesizedinstrument_flags
		sscore_sy_synthSoundParameterControls parameterControls;	// functions which return parameters for square and triangle waveforms. If NULL defaults are used
	} sscore_sy_synthesizedinstrumentinfo;


	/*!
	 @struct sscore_sy_controls
	 @abstract link to UI which defines playing parameters
	 */
	...
	/*!
	 @struct sscore_sy_controls_plus
	 @abstract link to UI which defines playing parameters with extra function
	 partStaffEnabled which permits playing of a single staff in a part
	 */
	...
	typedef struct sscore_synth_options
	{
		float volumeLaw; // value [0.0 .. 1.0] determines how much softer ppp is than fff. Higher values produce greater volume differences
		unsigned dummy[63]; // for undefined future enhancements
	} sscore_synth_options;
	...
	/*!
	 @function sscore_sy_createsynth_plus
	 @abstract create a new synth and return it
	 @param controls synth ui controls with extra functions defined above
	 @param options unused at present - can be null
	 @param key the key defining licenses owned by the user. If this key does not include a synth licence
	 then the synth will operate for a limited time only after the app is started
	 @return the synth
	 */
	EXPORT sscore_synth *sscore_sy_createsynth_plus(const sscore_sy_controls_plus *controls, const sscore_synth_options *options, const sscore_libkeytype *key);
	...
	/*!
	 ...
	 @abstract setup the synth with playdata.
	 @discussion You must add instruments before calling setup
	 ...
	 */
	EXPORT bool sscore_sy_pause(sscore_synth *synth);

	/*!
	 @function sscore_sy_ispaused
	 @param synth the synth from sscore_sy_createsynth
	 @return true if synth is paused
	 */
	EXPORT bool sscore_sy_ispaused(sscore_synth *synth);
	
	/*!
	 @function sscore_sy_resume
	 @abstract resume playing after pause
	 @param synth the synth from sscore_sy_createsynth
	 @return true if success
	 */
	EXPORT bool sscore_sy_resume(sscore_synth *synth);
	...
	/*!
	 ...
	 @return true if success
	 */
	EXPORT bool sscore_sy_setnextplaybar(sscore_synth *synth, int barindex, unsigned long long restart_time);
