...
#define sscore_kMaxChordNotes	32
#define sscore_kMaxDirectionsInGroup 10
#define sscore_kMaxDirectionTextStringLength 64 // in direction-type words element
#define sscore_kMaxDirectionWords 5
#define sscore_kMaxLyricTextStringLength 32 // a single note, single verse lyric
#define sscore_kMaxBarsInSystem	20

/*!
 @typedef sscore_directiontype_handle
 @abstract a handle for a direction-type inside a direction element
 */
...
/*!
	 @enum sscore_note_value_e
	 @abstract note values
 */
enum sscore_note_value_e
{
	sscore_note_value_none,sscore_square_breve,sscore_breve,sscore_semibreve_1,sscore_minim_2,sscore_crotchet_4,sscore_quaver_8,sscore_quaver_16,sscore_quaver_32,sscore_quaver_64,sscore_quaver_128
};
	
/*!
	 @enum sscore_note_stem_e
	 @abstract note stem types
 */
enum sscore_note_stem_e
{
	 sscore_note_undefined_stem, sscore_note_no_stem, sscore_note_up_stem, sscore_note_down_stem
};
...
enum sscore_component_type_e
{
	...
	sscore_comp_barline_thin,
	sscore_comp_barline_thick,
	sscore_comp_barline_double,
	sscore_comp_barline_pattern, // dot/dash..
	sscore_comp_barline_repeat_left,
	sscore_comp_barline_repeat_right,
	sscore_comp_barline_segno,
	sscore_comp_barline_coda,
	sscore_comp_barline_fermata,
	sscore_comp_harmony,
	sscore_comp_harmony_frame,
	sscore_comp_repeat_brace,
	sscore_comp_repeat_number,
	sscore_comp_part_name,
	sscore_comp_partgroup_name,
	sscore_comp_part_bracket,
	sscore_comp_multiple,
	sscore_comp_parent,
	sscore_comp_undefined = 1000
};
	
/*!
 @typedef sscore_component_handle
 @abstract a unique handle for an item in the layout
 @discussion The layout item is created from the score item and they have distinct identities
 */
...
typedef struct sscore_note
{
	...
	int type; // 2 = minim, 4 = crotchet etc.
	...
	bool grace;
	int beamcount;
	unsigned dummy[31]; // future
} sscore_note;

/*!
	@struct sscore_chordnote
	@abstract info for note which is part of a chord
 */
...
/*!
	@struct sscore_chord
	@abstract chord info
	@discussion assume all chord notes have same type, duration, dots and notations
 */
typedef struct sscore_chord
{
	...
	bool grace;
	int numnotes;
	struct sscore_chordnote notes[sscore_kMaxChordNotes];
...
/*!
	 ...
	 @abstract flags describing component
 */
enum sscore_comp_flags_e {
...
/*!
	 ...
	 @abstract identify type of control point
 */
enum sscore_comp_controlpoint_e
{
...
typedef struct sscore_timesig_detail
{
	// ..overlay of sscore_timesig
	int numbeats;
	int beattype;
	// ..
	enum sscore_timesig_type type;

	unsigned dummy[32];// future
} sscore_timesig_detail;
...
/*!
	...
	@description use sscore_con_itemforhandle to convert this to a full sscore_item
*/
...
/*!
	 ...
	 @abstract information about items in part/bar
 */
...
typedef struct sscore_component
{
	...
	int partindex;
	int barindex;	// bar index where displayed
	sscore_rect rect;
	...
	int staffindex;
	sscore_item_handle inner_h;
	unsigned flags;				// enum sscore_comp_flags_e
	enum sscore_comp_controlpoint_e controlid;
	int definedInBarIndex;		// bar index where defined in xml
	int multiItemIndex;	// eg index of lyric
	enum sscore_item_type_e item_tp;
} sscore_component;

/*!
	 @struct sscore_systemgroup
	 @abstract information about group of (selected) items in bar
 */
typedef struct sscore_systemgroup
{
	int num;
	sscore_bargroup bargroups[sscore_kMaxBarsInSystem];
	unsigned dummy[8]; // future
} sscore_systemgroup;
...
/*!
 @struct sscore_con_directiontype
 @abstract corresponds to direction-type element in musicxml
 */
typedef struct sscore_con_directiontype
{
	...
	int partindex;
	int barindex;
	sscore_item_handle dir_h;
	sscore_directiontype_handle dirType_h;
} sscore_con_directiontype;

/*!
 @struct sscore_directiongroup
 @abstract list of sscore_con_directiontype
 */
typedef struct sscore_con_directiongroup
{
	int num;
	sscore_con_directiontype directions[sscore_kMaxDirectionsInGroup];
	
	unsigned dummy[15]; // future
} sscore_con_directiongroup;

/*!
	 @struct sscore_con_formattedtext
	 @abstract corresponds to formatted-text element in musicxml
 */
...
/*!
 @struct sscore_con_words_directiontype
 @abstract corresponds to direction-type words element in musicxml
 */
...
enum sscore_lyric_syllabic { // corresponding to MusicXML syllabics
	sscore_lyr_syl_none,
	sscore_lyr_syl_single,
	sscore_lyr_syl_begin,
	sscore_lyr_syl_end,
	sscore_lyr_syl_middle};

typedef struct sscore_con_lyric
{
	int partindex;
	int barindex;
	sscore_item_handle note_h;
	int lyric_num; // 0.. index in list of lyrics for note
	char text[sscore_kMaxLyricTextStringLength];
	enum sscore_lyric_syllabic syllabic;
	float fontSize; // from defaults in XML header
	bool bold;
	bool italic;
	int staffindex;
	unsigned dummy[31]; // future
...
/*!
	 @function sscore_con_closefeatures
	 @abstract return information (in result) about components in system which are within distance of point, sorted by increasing distance
	 @discussion licence contents_capable is required to use this function
	 @param sc the score
	 @param sys the system being interrogated
	 @param p the coordinates of the point
	 @param distance the radius around p for items to intersect
	 @param result pointer to client-allocated sscore_componentgroup description (array) of components which intersect radius around point
	 @return sscore_error = sscore_NoError and result filled in, or an appropriate error value
*/
EXPORT enum sscore_error sscore_con_closefeatures(const sscore *sc, const sscore_system *sys, const sscore_point *p, float distance,
													sscore_componentgroup *result);	

/*!
	 @function sscore_con_featureswithin
	 @abstract return information (in result) about components in system which are enclosed within the rectangle defined by r
	 @discussion licence contents_capable is required to use this function
	 @param sc the score
	 @param sys the system being interrogated
	 @param r the enclosing rectangle
	 @param result pointer to client-allocated sscore_componentgroup description (array) of components which intersect radius around point
	 @return sscore_error = sscore_NoError and result filled in, or an appropriate error value
 */
EXPORT enum sscore_error sscore_con_featureswithin(const sscore *sc, const sscore_system *sys, const sscore_rect *r,
													   sscore_componentgroup *result);
	
/*!
	 @function sscore_con_itemsforcomponents
	 @abstract return information (in result) about the items which have produced the set of components
	 @discussion licence contents_capable is required to use this function
	 @param sc the score
	 @param components the components eg returned from sscore_con_featureswithin
	 @param result pointer to client-allocated sscore_systemgroup description (array) of bar groups containing items
	 @return sscore_error = sscore_NoError and result filled in, or an appropriate error value
*/
EXPORT enum sscore_error sscore_con_itemsforcomponents(const sscore *sc, const sscore_componentgroup *components,
													  sscore_systemgroup *result);
...
/*!
	@function sscore_con_directionsatitem
	@abstract get the direction(s) which occur at the same time as the item (note or direction) defined by item_h
	@param sc the score
	@param sys the system
	@param partindex the part index [0..]
	@param barindex the bar index [0..]
	@param item_h the component handle
	@param result will be filled in with sscore_con_directiontype identification(s). use sscore_con_direction_XXX to query.
	@return any error
 */
EXPORT enum sscore_error sscore_con_directionsatitem(const sscore *sc, const sscore_system *sys,
													 int partindex, int barindex, sscore_item_handle item_h,
													 sscore_con_directiongroup *result);


/*!
	@function sscore_con_directionforid
	@abstract get the direction-type with the id dir_type_h
	@param sc the score
	@param partindex the part index [0..]
	@param barindex the bar index [0..]
	@param dir_type_h the handle of the direction-type inside the direction
	@return the direction-type or .tp = sscore_dir_undefined if not found
 */
EXPORT sscore_con_directiontype sscore_con_directionforid(const sscore *sc,
														  int partindex, int barindex,
														  sscore_directiontype_handle dir_type_h);

/*!
	@function sscore_con_componentsfordirectiontype
	@abstract get the components for the given direction-type
	@param sys the system
	@param dir the directiontype from sscore_con_directionsatnote
	@param result pointer to client-allocated sscore_componentgroup description (array) of layout components for the direction-type
	@return any error
 */
EXPORT enum sscore_error sscore_con_componentsfordirectiontype(const sscore_system *sys,
															   const sscore_con_directiontype *dir,
															   sscore_componentgroup *result);

/*!
	@function sscore_con_direction_staffindex
	@abstract get the staff index for the direction - 0 is top or only staff, 1 is bottom staff
	@param sc the score
	@param dir the direction type (corresponding to direction.direction-type)
	@return the staff index
*/
EXPORT int sscore_con_direction_staffindex(const sscore *sc, const sscore_con_directiontype *dir);

/*!
	@function sscore_con_direction_isdirective
	@abstract get the directive flag for the direction
	@param sc the score
	@param dir the direction type (corresponding to direction.direction-type)
	@return true if this is a directive (aligned to left of time signature)
 */
EXPORT bool sscore_con_direction_isdirective(const sscore *sc, const sscore_con_directiontype *dir);
	
/*!
	@function sscore_con_direction_placement
	@abstract get the  placement - above staff or below
	@param sc the score
	@param dir the direction type (corresponding to direction.direction-type)
	@return the placement
 */
EXPORT enum sscore_placement_e sscore_con_direction_placement(const sscore *sc, const sscore_con_directiontype *dir);
	
/*!
	@function sscore_con_direction_words
	@abstract get the words information for a direction-type words
	@param sc the score
	@param dir the direction type (corresponding to direction.direction-type)
	@return the words for a direction-type words
 */
EXPORT sscore_con_words_directiontype sscore_con_direction_words(const sscore *sc, const sscore_con_directiontype *dir);

/*!
	 @function sscore_con_getlyric
	 @abstract get the words information for a direction-type words
	 @param sc the score
	 @param partindex the part index
	 @param barindex the bar index
	 @param note_h the note handle
	 @param lyric_num the number of the lyric or 0 to get the first
	 @return result. If no lyric found note_h = 0 
 */
EXPORT sscore_con_lyric sscore_con_getlyric(const sscore *sc, int partindex, int barindex, sscore_item_handle note_h, int lyric_num);
...
/*!
	...
	@abstract return the xml description of the bar (&lt measure&gt;) in the part
	@discussion The xml is regenerated, so is not necessarily identical to the corresponding file section.
	On return the XML is a C-style NULL-terminated sequence of UTF-8 characters in the buffer
	@discussion licence contents_capable is required to use this function
	@param sc the score
...
/*!
	 @function sscore_con_xmlforscore
	 @abstract return the (reconstructed) xml for the entire score
	 @discussion licence contents_capable is required to use this function
	 @param sc the score
	 @param buffer pointer to buffersize writeable bytes
	 @param buffersize the total allocated size of the buffer
	 @return sscore_error = sscore_NoError if item found and XML returned in buffer, or an appropriate error value
*/
EXPORT enum sscore_error sscore_con_xmlforscore(const sscore *sc,
												char *buffer, int buffersize);
	
/*!
	 @function sscore_con_clefForComponentItem
	 @abstract return the clef applying to the item owning the component
	 @param sc the score
	 @param comp the component
	 @return the clef
 */
EXPORT enum sscore_clef_type_e sscore_con_clefForComponentItem(const sscore *sc, const sscore_component *comp);

/*!
	@function sscore_con_clefApplyingAt
	@abstract return the clef applying at the start of the given bar in the given part/staff
	@param sc the score
 ...
 */
EXPORT enum sscore_clef_type_e sscore_con_clefApplyingAt(const sscore *sc, int partIndex, int barIndex, int staffIndex);

EXPORT int sscore_con_keyFifthsApplyingAt(const sscore *sc, int partIndex, int barIndex, int staffIndex);

EXPORT sscore_timesig_detail sscore_con_timeSigApplyingAt(const sscore *sc, int partIndex, int barIndex, int staffIndex);
	
/*!
 @function sscore_con_numLyricLinesInPart
 @abstract get the number of lyric lines in the part
 @discussion This is not the same as sscore_system_numLyricLines which returns the number
	of lyric lines in an individual system
 @param sc the sscore pointer returned from sscore_load*
 @param partIndex the 0-based part index
 @param staffIndex the 0-based staff index (0 is top or only staff in part)
 @return the number of lyric lines in this part
 */
EXPORT int sscore_con_numLyricLinesInPart(sscore *sc, int partIndex, int staffIndex);

/*!
 @function sscore_con_getLyricsInPart
 @abstract get all the lyrics in the part/staff/lyric line
 @discussion each lyric is separated by a space or by a hyphen according to the syllabic settings
 @param sc the sscore pointer returned from sscore_load*
 @param partIndex the 0-based part index
 @param staffIndex the 0-based staff index (0 is top or only staff in part)
 @param lyricLineIndex the 0-based lyricindex (0 is top or only lyric in part)
 @param buffer receives the UTF-8 lyrics (space separates notes, hyphen indicates syllabics)
 @param buffersize must be set to the allocated size of the buffer
 @return any error
 */
EXPORT enum sscore_error sscore_con_getLyricsInPart(sscore *sc, int partIndex, int staffIndex, int lyricLineIndex, char buffer[], int buffersize);

/*!
 @function sscore_con_firstLyric
 @abstract return the information about the first lyric in the part
 @discussion set the partIndex, staffIndex and lyricLine as required to get the lyric on the first note in the part.
 Call sscore_con_nextLyric repeatedly with the last returned struct to get lyrics on subsequent notes through the part.
 @param sc the score
 @param partIndex the part index
 @param staffIndex the part index
 @param lyricLine the lyric line
 @return information about the next lyric in the part
 */
EXPORT sscore_con_lyric sscore_con_firstLyric(const sscore *sc, int partIndex, int staffIndex, int lyricLine);

/*!
 @function sscore_con_nextLyric
 @abstract return the information about the next lyric after previous
 @param sc the score
 @param previous use result from sscore_con_firstLyric or a previous call to sscore_con_nextLyric.
 @return information about the next lyric in the part. text is empty and note_h = 0 after last lyric in part
*/
EXPORT sscore_con_lyric sscore_con_nextLyric(const sscore *sc, const sscore_con_lyric *previous);

/*!
 @function sscore_con_lyricOnNote
 @abstract return the information about a lyric on a note
 @param sc the score
 @param partIndex the part index
 @param barIndex the bar index
 @param lyricLine the lyric line - 0 is first/top lyric
 @param note_h the note handle
 @return information about the lyric on the note
 */
EXPORT sscore_con_lyric sscore_con_lyricOnNote(const sscore *sc, int partIndex, int barIndex, int lyricLine, sscore_item_handle note_h);
	
/*!
 @function sscore_con_componentForLyric
 @abstract return the sscore_component for the lyric
 @param sys the system
 @param lyric eg from sscore_con_firstLyric or sscore_con_nextLyric
 @return the sscore_component for the lyric (tp = 0 if not found)
 */
EXPORT sscore_component sscore_con_componentForLyric(const sscore_system *sys, const sscore_con_lyric *lyric);
...
/*!
 ...
 @discussion buffer should point to a read-write area of size buffersize
 */
EXPORT enum sscore_error sscore_stringfortimesig(const sscore_timesig_detail *ts, char *buffer, int buffersize);
/*!
