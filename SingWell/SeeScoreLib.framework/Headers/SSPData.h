...
/*!
 @method slashGracesPerCrotchet
 @return the length of a grace-slash as a fraction (denominator) of a crotchet (quarter note). 0 = use default
 */
-(int)slashGracesPerCrotchet;

@end

/*! @protocol SSMidiControls
 @abstract allow enabling of individual parts, upper/lower staff, and metronome in MIDI file
 */
@protocol SSMidiControls <NSObject>

-(bool)partEnabled:(int)partIndex; // return true to output notes in this part into MIDI file

-(int)midiProgramForPart:(int)partIndex; // define MIDI program change value for part if > 0

-(bool)partStaffEnabled:(int)partIndex staffIndex:(int)staffIndex; // return true to output notes in part and staff

-(bool)metronomeEnabled; // return true to output notes for metronome ticks in percussion channel 10

-(int)midiKeyForMetronome; // return MIDI key number for metronome tick in percussion channel 10 (see MIDI Spec 1.1, Appendix 1.5 - General MIDI Percussion Key Map)

-(float)partVolume:(int)partIndex; // return 0.0 .. 1.0

-(float)metronomeVolume; // return 0.0 .. 1.0
...
/*!
 @property midiPitch
 @abstract the midi pitch value of the note 60 = C4.
 @discussion rests are returned as SSPDNote in the SSPData with midiPitch = 0
 @discussion If the note in the XML is unpitched and has a defined instrument id then a lookup
 is performed in the table of |score-instrument| in |score-part| in the xml file to find an
 instrument name, and if the identical name is found in the "General MIDI Percussion Key Map"
 defined in the MIDI Spec 1.1 Appendix 1.5, the midi pitch value from this table is returned
 in midiPitch. In this way it can play a set of standard MIDI percussion sound samples directly 
 */
...
/*!
 @property loopBack
 @abstract if this is set we play in a loop over the bars array
 */
...
/*!
 ...
 @param partIndex the part index
 @param barIndex the bar index
 @param noteId an object implementing the SSUTempo protocol allowing the user to set the tempo while playing.
 @return the SSPData
 */
...
/*!
 @method createMidiFile:withControls:
 @abstract create a MIDI file from this play data with the given full pathname and controls
 @param filePath full file pathname
 @param controls object to control parts enabled and metronome
 @return any error
 */
-(enum sscore_error)createMidiFile:(NSString*)filePath withControls:(id<SSMidiControls>)controls;
...
/*!
 @method updateTempo
 @abstract notification that the tempo (slider?) in the UI has changed so it can be reread
  */
-(void)updateTempo;
